...
const impactConfig = {
    ...
    'Low': { color: 'text-sky-700', bg: 'bg-sky-100', border: 'border-sky-200' },
};
...
const BusinessReport: React.FC<BusinessReportProps> = ({ result, task, onReset }) => {
    ...
    return (
        <div className="w-full max-w-5xl mx-auto p-8 bg-white border border-slate-200/80 rounded-xl shadow-sm animate-fade-in space-y-8">
             <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div>
                    <h1 className="text-3xl md:text-4xl font-bold text-slate-900">Business Validation Report</h1>
                    <p className="text-slate-500 mt-1">Task: <span className="font-semibold text-slate-700">{task}</span></p>
                </div>
                <button
                onClick={onReset}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
                <div className="md:col-span-1 flex flex-col items-center justify-center bg-slate-50 p-6 rounded-xl border border-slate-200">
                    <h2 className="text-lg font-semibold text-slate-700 mb-4">Simulated Conversion Rate</h2>
                    <div className="relative w-48 h-48">
                         <svg className="w-full h-full" viewBox="0 0 36 36">
                            ...
                            <path
                                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                                className="text-violet-500 drop-shadow-[0_0_5px_rgba(139,92,246,0.5)] transition-all duration-1000"
                                fill="none"
                                stroke="currentColor"
                                strokeWidth="3"
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                            <span className="text-5xl font-bold text-slate-800">{conversionRate}<span className="text-3xl text-slate-500">%</span></span>
                        </div>
                    </div>
                </div>
                <div className="md:col-span-2 bg-slate-50 p-6 rounded-xl border border-slate-200">
                    <h2 className="text-lg font-semibold text-slate-700 mb-4">Top Friction Points Causing Drop-off</h2>
                             ...
                             return (
                                <li key={index} className={`p-4 rounded-lg border-l-4 ${config.bg} ${config.border}`}>
                                    <div className="flex items-start">
                                        ...
                                        <p className="text-slate-700">{item.point}</p>
                                    </div>
                                </li>
                            )
                        })}
            ...
            <div className="text-center pt-4">
                <button
                    ...
                    className="inline-flex items-center gap-2 px-6 py-3 text-md font-semibold text-white bg-violet-600 rounded-lg hover:bg-violet-700 transition-all duration-200 shadow-md hover:shadow-lg"
                >
